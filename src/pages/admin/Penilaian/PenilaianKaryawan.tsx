...
import type { AssessmentCategory, AssessmentDetailInput, SubordinateEmployee } from "../../../types/assessments.types";
    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <button key={star} type="button"
          ...
          <svg className={`h-7 w-7 transition ${(hover || value) >= star ? "text-amber-400" : "text-gray-200 dark:text-gray-700"}`}
...
// ── Evaluation Modal ──────────────────────────────────────────
function EvaluationModal({ isOpen, onClose, employee, existingAssessmentId, period, onSaved }: {
  isOpen: boolean; onClose: () => void; employee: SubordinateEmployee | null;
  existingAssessmentId: string | null; period: string; onSaved: () => void;
}) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setNotes(""); setScores({});
    assessmentsService.getCategories({ isActive: "true" }).then((cats) => {
      setCategories(cats);
    ...
    if (existingAssessmentId) {
      assessmentsService.getIndividualById(existingAssessmentId).then((data) => {
        setNotes(data.generalNotes);
        const s: Record<string, number> = {};
        data.categories.forEach((c) => { s[c.categoryId] = c.score; });
        setScores(s);
      }).catch(() => {});
    }
  }, [isOpen, existingAssessmentId]);

  const allScored = categories.length > 0 && categories.every((c) => (scores[c.id] ?? 0) > 0);

  const handleSave = async () => {
    if (!employee || !allScored || !notes.trim()) return;
    const details: AssessmentDetailInput[] = categories.map((cat) => ({
      categoryId: cat.id, categoryName: cat.name, score: scores[cat.id] ?? 1,
    }));
    setSaving(true);
    try {
      if (existingAssessmentId) {
        await assessmentsService.update(existingAssessmentId, { generalNotes: notes, details });
      } else {
        await assessmentsService.submit({ evaluateeId: employee.employeeId, period, generalNotes: notes, details });
      }
      onSaved(); onClose();
    ...
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-lg p-0 overflow-hidden">
      ...
      <div className="px-6 py-5">
        {loading ? (
          ...
          <>
            <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-gray-400">Indikator Penilaian</p>
            <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
              {categories.map((cat) => (
                <div key={cat.id} className="flex items-center justify-between rounded-xl border border-gray-100 bg-white px-4 py-3 dark:border-gray-800 dark:bg-gray-900">
                  <div>
                  ...
                  <StarRating value={scores[cat.id] ?? 0} onChange={(v) => setScores((p) => ({ ...p, [cat.id]: v }))} />
                </div>
              ))}
            </div>
            <div className="mt-4">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-gray-400">Catatan / Feedback *</p>
        ...
        <div className="mt-4 flex gap-3">
          <button onClick={onClose} disabled={saving} className="flex-1 rounded-xl border border-gray-200 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300">Batal</button>
          <button onClick={handleSave} disabled={saving || !allScored || !notes.trim()}
            ...
            {saving ? "Menyimpan..." : existingAssessmentId ? "Simpan Perubahan" : "Simpan & Lanjut →"}
          </button>
        </div>
      </div>
    </Modal>
...
// ── Employee Card ─────────────────────────────────────────────
function EmployeeCard({ emp, onBeriNilai, onEdit }: {
  emp: SubordinateEmployee; onBeriNilai: (e: SubordinateEmployee) => void; onEdit: (e: SubordinateEmployee) => void;
}) {
  const navigate = useNavigate();
  return (
    <div className={`relative rounded-2xl border p-4 transition hover:shadow-sm ${emp.isReviewed ? "border-green-100 dark:border-green-500/20" : "border-gray-200 dark:border-gray-700"} bg-white dark:bg-white/[0.03]`}>
      <span className={`absolute right-3 top-3 rounded-full px-2 py-0.5 text-[10px] font-bold ${emp.isReviewed ? "bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400" : "bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400"}`}>
      ...
      <div className="mt-3 flex items-center justify-between">
        {emp.isReviewed ? (
          <div className="flex gap-2">
            <button onClick={() => navigate(`/admin/laporan-individu/${emp.assessmentId}`)}
          ...
          <button onClick={() => onBeriNilai(emp)}
...
export default function PenilaianKaryawan() {
  ...
  useEffect(() => { fetchDashboard(period); }, [period]); // eslint-disable-line

  const filtered = subordinates.filter((e) => tab === "pending" ? !e.isReviewed : tab === "done" ? e.isReviewed : true);
  const selesai = subordinates.filter((e) => e.isReviewed).length;
  const pending = subordinates.filter((e) => !e.isReviewed).length;
  const progress = subordinates.length ? Math.round((selesai / subordinates.length) * 100) : 0;

  return (
    <>
      <PageMeta title="Penilaian Karyawan" description="Dashboard evaluasi kinerja karyawan" />
      <PageBreadcrumb pageTitle="Penilaian Karyawan" />
      <div className="space-y-6">
        {/* Header */}
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Dashboard Penilaian</h1>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Pantau kemajuan evaluasi kinerja tim Anda bulan ini.</p>
          </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
            <div className="mb-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-50 dark:bg-brand-500/10">
                  <svg className="h-5 w-5 text-brand-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>
                </div>
                <div>
                  <p className="font-semibold text-gray-700 dark:text-gray-200">Kemajuan Penilaian</p>
                  <p className="text-xs text-gray-400">Menilai {selesai} dari {subordinates.length} staf bulan ini</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              {[
                ...
                { label: "Rata-rata Skor", value: dashboardStats.rataRataSkor.toFixed(1), cls: "text-gray-700 dark:text-white" },
                { label: "Deadline", value: dashboardStats.deadline, cls: dashboardStats.daysUntilReset <= 3 ? "text-red-600" : "text-gray-700 dark:text-white" },
              ].map(({ label, value, cls }) => (
                <div key={label}>
        ...
        <div className="flex gap-6 border-b border-gray-100 dark:border-gray-800">
          {(["all", "pending", "done"] as Tab[]).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              ...
              {t === "all" ? `SEMUA STAF (${subordinates.length})` : t === "pending" ? `BELUM DINILAI (${pending})` : `SUDAH DINILAI (${selesai})`}
            </button>
          ))}
        </div>
        ...
        {dashboardLoading ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-44 animate-pulse rounded-2xl bg-gray-100 dark:bg-gray-800" />)}
          </div>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filtered.map((emp) => (
              <EmployeeCard key={emp.employeeId} emp={emp}
                onBeriNilai={(e) => { setModalEmp(e); setModalAssessmentId(null); }}
                onEdit={(e) => { setModalEmp(e); setModalAssessmentId(e.assessmentId); }} />
            ))}
            {filtered.length === 0 && <div className="col-span-full py-12 text-center text-sm text-gray-400">Tidak ada karyawan ditemukan.</div>}
          </div>
        )}
      </div>

      <EvaluationModal
        isOpen={!!modalEmp} onClose={() => { setModalEmp(null); setModalAssessmentId(null); }}
        employee={modalEmp} existingAssessmentId={modalAssessmentId}
        period={period} onSaved={() => fetchDashboard(period)}
      />
    </>
  );
}
