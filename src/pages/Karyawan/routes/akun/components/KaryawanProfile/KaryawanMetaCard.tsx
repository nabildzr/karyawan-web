...
export default function KaryawanMetaCard() {
  // const { isOpen, openModal, closeModal } = useModal();

  const { employeeProfile, user } = useAuthContext();

  const displayName = employeeProfile?.fullName ?? "User";
  const displayRole = user?.role ?? "No Role";
  const displayPosition = employeeProfile?.position?.name ?? "-";
  const displayDivision = employeeProfile?.position?.division?.name ?? "-";
