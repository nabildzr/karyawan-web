...
const ScheduleCard = ({ item, onOpenDetail }: ScheduleCardProps) => {
  ...
  return (
    <article
      ...
      className={`rounded-2xl border p-4 transition ${statusStyle.container} ${onOpenDetail ? "cursor-pointer hover:shadow-theme-xs" : ""}`}
    >
      <div className="flex items-start gap-4">
        <div className="min-w-[64px] border-r border-gray-200 pr-4 text-center">
          <p
            className={`text-xs font-semibold tracking-wider ${statusStyle.dayText}`}
          >
            {item.dayShort}
          </p>
          <p
            className={`text-4xl font-bold leading-none ${statusStyle.dayText}`}
          >
            {item.dayNumber}
          </p>
        </div>
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between gap-3">
            <h3 className={`text-lg font-semibold ${statusStyle.titleText}`}>
              {item.shiftName}
            </h3>
            <div className="flex items-center gap-2">
              <span
                className={`rounded-full px-2.5 py-1 text-xs font-semibold ${statusStyle.badge}`}
              >
          ...
          <p className={`text-base ${statusStyle.bodyText}`}>
            {item.shiftTime}
          </p>
          {item.workDurationLabel && (
            <p className={`mt-1 text-xs font-semibold ${statusStyle.bodyText}`}>
              Durasi kerja: {item.workDurationLabel}
            </p>
          )}
          <p className={`mt-1 text-xs ${statusStyle.bodyText}`}>
            {item.dateLabel}
          </p>
          <p className={`mt-2 text-sm ${statusStyle.bodyText}`}>
            {item.description}
          </p>
          {onOpenDetail && (
            <p className={`mt-3 text-xs font-semibold ${statusStyle.bodyText}`}>
